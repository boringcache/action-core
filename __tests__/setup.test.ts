...
describe('action-core', () => {
    ...
    it('returns true when CLI is available', async () => {
      mockedExec.exec.mockImplementation(async (cmd, args, options) => {
        if (options?.listeners?.stdout) {
          options.listeners.stdout(Buffer.from('boringcache v1.7.1'));
        }
        return 0;
      });
  ...
  describe('ensureBoringCache', () => {
    ...
    it('downloads CLI when not available', async () => {
      ...
      expect(mockedTc.downloadTool).toHaveBeenCalledWith(
        'https://github.com/boringcache/cli/releases/download/v1.7.1/boringcache-linux-amd64'
      );
      expect(mockedTc.downloadTool).toHaveBeenCalledWith(
        'https://github.com/boringcache/cli/releases/download/v1.7.1/SHA256SUMS'
      );
    ...
    it('uses cached version when available', async () => {
      ...
      expect(mockedTc.find).toHaveBeenCalledWith('boringcache', '1.7.1');
      expect(mockedTc.downloadTool).not.toHaveBeenCalled();
  ...
  describe('checksum verification', () => {
    ...
    it('skips verification when verify: false', async () => {
      await ensureBoringCache({ version: 'v1.7.1', verify: false });
    ...
    it('throws error when checksum not found in SHA256SUMS', async () => {
      ...
      await expect(ensureBoringCache({ version: 'v1.7.1' })).rejects.toThrow(
  ...
  describe('getToolCacheInfo', () => {
    it('returns cache info with correct tool name and version', () => {
      ...
      const info = getToolCacheInfo('v1.7.1');
    ...
    it('returns cache path when tool is cached', () => {
      process.env.RUNNER_OS = 'Linux';
      process.env.RUNNER_ARCH = 'X64';
      mockedTc.find.mockReturnValue('/opt/hostedtoolcache/boringcache/1.7.1/x64');

      const info = getToolCacheInfo('v1.7.1');

      expect(info.cachePath).toBe('/opt/hostedtoolcache/boringcache/1.7.1/x64');
    });
    ...
    it('returns correct cache pattern', () => {
      ...
      expect(info.cachePattern).toBe('/custom/tool/cache/boringcache/1.7.1*');
    });
    ...
    it('returns platform-specific cache key', () => {
      ...
      expect(info.cacheKey).toBe('boringcache-1.7.1-linux-arm64');
    });
  });
  ...
  describe('automatic caching', () => {
    it('restores from actions/cache on startup', async () => {
      mockedExec.exec.mockRejectedValueOnce(new Error('not found'));
      mockedCache.restoreCache.mockResolvedValue('boringcache-1.7.1-linux-x64');
      mockedTc.find.mockReturnValue('/opt/hostedtoolcache/boringcache/1.7.1/x64');
    ...
    it('saves to actions/cache after download', async () => {
      ...
      expect(mockedCache.saveCache).toHaveBeenCalledWith(
        expect.any(Array),
        expect.stringContaining('boringcache-1.7.1-linux-x64')
      );
    });
    ...
    it('skips caching when cache option is false', async () => {
      ...
      await ensureBoringCache({ version: 'v1.7.1', cache: false });
